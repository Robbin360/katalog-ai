...
comment on column public.shopify_products.publish_attempts is 'Número de intentos de publicación Shopify para el intento actual.';
comment on column public.shopify_products.publish_next_retry_at is 'Próximo instante UTC para reintentar un fallo transitorio.';
comment on column public.shopify_products.publish_error_code is 'Código estable de error de publicación, sin secretos.';
comment on column public.shopify_products.publish_error_stage is 'Etapa: preflight, shopify_request, shopify_verify o local_finalize.';
comment on column public.shopify_products.publish_error_retryable is 'Indica si Auto-Pilot o el usuario pueden reintentar sin corregir configuración.';
comment on column public.shopify_products.publish_error_at is 'Última fecha UTC en que se registró un error de publicación.';
comment on column public.shopify_products.publish_error_details is 'Detalles sanitizados para diagnóstico, nunca tokens ni headers secretos.';

create index if not exists shopify_products_publish_retry_idx
  on public.shopify_products (publish_next_retry_at)
  where publish_error_retryable = true and publish_next_retry_at is not null;

-- Registra todos los fallos sin degradar un estado final válido.
-- Si Shopify ya confirmó, p_preserve_ready=true mantiene READY_TO_PUBLISH para
-- que el cierre local pueda reintentarse. OPTIMIZED nunca se pisa.
create or replace function public.record_publish_failure(
  ...
  p_preserve_ready boolean default false
)
returns table (recorded boolean, reason text, audit_status text)
language plpgsql
security definer
...
begin
  ...
  if not found then
    return query select false, 'product_not_found'::text, null::text;
    return;
  end if;
  if v_owner_id is distinct from p_user_id then
    return query select false, 'user_mismatch'::text, v_status;
    return;
  end if;
  if v_status = 'OPTIMIZED' then
    return query select false, 'already_optimized'::text, v_status;
    return;
  end if;

  v_new_status := case when p_preserve_ready and v_status = 'READY_TO_PUBLISH'
                       then 'READY_TO_PUBLISH' else 'ERROR' end;
  v_details := case when jsonb_typeof(coalesce(p_details, '{}'::jsonb)) = 'object'
                    then coalesce(p_details, '{}'::jsonb)
                    else jsonb_build_object('value', left(coalesce(p_details, '{}'::jsonb)::text, 2000)) end;

  update public.shopify_products
  set audit_status = v_new_status,
      ...
      publish_error_retryable = coalesce(p_retryable, false),
      publish_error_at = now(),
      publish_error_details = v_details,
      publish_next_retry_at = case when p_retryable then p_next_retry_at else null end,
