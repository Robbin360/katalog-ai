...
-- Retira la RPC, el índice y la constraint, pero conserva las columnas
-- publish_* para no borrar diagnóstico ni datos de producción.
-- La eliminación de columnas requiere una migración posterior explícita,
-- después de verificar que ningún consumidor las usa.

begin;

revoke all on function public.record_publish_failure(
  uuid, bigint, text, text, boolean, text, jsonb, timestamptz, boolean
) from public;
revoke all on function public.record_publish_failure(
  uuid, bigint, text, text, boolean, text, jsonb, timestamptz, boolean
) from anon;
...
drop function if exists public.record_publish_failure(
  uuid, bigint, text, text, boolean, text, jsonb, timestamptz, boolean
);
