...
import { Package, Loader2, Plus, Upload, Link as LinkIcon, MoreVertical, LogOut, Settings, CreditCard, Trash2, AlertCircle } from "lucide-react"
...
export default function SidebarList() {
  ...
  // 2. Fetch Productos (Con Auto-refresco inteligente)
  const { data: products, isLoading } = useQuery({
    ...
    // Refresca cada 2s si hay algo procesando, sino cada 30s
  ...
  const handleDelete = async (id: number, e: React.MouseEvent) => {
    e.stopPropagation() // Para que no seleccione el producto al hacer clic en borrar
  ...
  // Renderizado de Lista (Extraído para limpieza)
      ...
      <div className="p-3 border-b border-zinc-800 bg-zinc-900">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="w-full bg-zinc-950 border border-zinc-800 grid grid-cols-2">
            <TabsTrigger value="inventory" className="text-xs data-[state=active]:bg-zinc-800 data-[state=active]:text-white text-zinc-500"><span>Inventory</span></TabsTrigger>
            <TabsTrigger value="new" className="text-xs flex gap-2 items-center data-[state=active]:bg-zinc-800 data-[state=active]:text-white text-zinc-500"><Plus className="w-3 h-3" /> <span>New Asset</span></TabsTrigger>
          </TabsList>
        </Tabs>
      </div>
      ...
      <div className="p-3 border-t border-zinc-800 bg-zinc-900 mt-auto">
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-60 bg-zinc-900 border-zinc-800 text-zinc-300 ml-2" align="start" side="top">
            ...
            <DropdownMenuItem onClick={() => router.push('/account')} className="text-xs px-2 py-2 cursor-pointer focus:bg-zinc-800 focus:text-white rounded-md">
